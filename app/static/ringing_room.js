...
bell_circle = new Vue({
    ...
    mounted: function() {
        ...
        function keyhandler_make_call(e, key, code) {
        ...
        function keyhandler_rotate_circle(e, key, code) {
        ...
        function keyhandler_ring_bells(e, key, code) {
        ...
        function keyhandler_stop_if_chat(e, key, code) {
            if ($("#chat_input_box").is(":focus")) {
                if (key == 'Escape') {
                    $('#chat_input_box').blur();
                }
                return true; // disable hotkeys when typing
            }

            if ($('#report_box').hasClass('show')) {
                return true; // disable hotkeys when the report is active
            }
            return false;
        }

        function keyhandler_special_keydown(e, key, code) {
            //Prevent duplicates
        ...
        function keyhandler_special_keyup(e, key, code) {
            // Do a special thing to prevent space from pressing focused buttons
            if (e.which == 32 && !$('#chat_input_box').is(':focus')) {
                e.preventDefault();
            }

            bell_circle.keys_down.splice(bell_circle.keys_down.indexOf(key),1)
            return false;
        }

        // Takes an array of functions taking three arguments (e, key, code)
        // Runs each until one returns true to indicate it has fully handled the key press
        function keyPressListener(keyhandlers) {
            return (e) => {
                e = e || window.event;
        ...
        if (!window.tower_parameters.anonymous_user) {
            ...
            const keyup_handlers = this.key_up_down_ringing
                ? [keyhandler_stop_if_chat, keyhandler_special_keyup, keyhandler_ring_bells]
                : [keyhandler_special_keyup]

            window.addEventListener('keyup', keyPressListener(keyup_handlers));

            $('[data-toggle="tooltip"]').tooltip();

            window.addEventListener('keydown', keyPressListener([
                keyhandler_stop_if_chat, keyhandler_special_keydown, keyhandler_ring_bells, keyhandler_rotate_circle, keyhandler_make_call
            ]));
        }
    },
